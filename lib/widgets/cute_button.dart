...
class CuteButtonWrapperState extends State<CuteButtonWrapper> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      child: widget.dragConfig == DragConfig.fixed
          ? buildButton(context)
          : Draggable(
              axis: widget.axis,
              ...
              onDragEnd: (d) {
                ...
                setState(() {
                  buttonStatus = _ButtonStatus.downToUp;
                  reaction =
                      d.wasAccepted ? Reaction.success : Reaction.failure;
                });
              }),
    );
  }
    ...
    if (widget.child.cuteButtonType == CuteButtonType.cuteButton)
      return SizedBox(
        ...
        child: Stack(
          ...
          children: [
            widget.child.highlight
                ? SizedBox(
                    // width: widget.size.width,
                    height: widget.size.height,
                    child: Image(
                      image: AssetImage("assets/highlight.gif"),
                      alignment: Alignment.center,
                      fit: BoxFit.fitHeight,
                    ))
                : Container(),
            Opacity(
              opacity: widget.child.highlight ? 0.9 : 1.0,
              child: Center(
                child: AspectRatio(
                  aspectRatio: 1.0,
                  child: RaisedButton(
